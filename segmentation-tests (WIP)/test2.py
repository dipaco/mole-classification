...
def magic(segmentationProcess=True, saveSegmentation=True, featuresProcess=True):
    ...
    for image in fnmatch.filter(os.listdir('imgs'), '*.bmp'):
        ...
        if featuresProcess:
            ...
            if (jona >= mean_jona) and (np.sum(Isegmented255) > 0) and featurextraction:
            ...
            elif (jona >= mean_jona) and (np.sum(Isegmented255) > 0) and testing:
                print(image[: -4])
